...
std::string ToPercentage(std::chrono::duration<Rep, Period> value, std::chrono::duration<Rep, Period> total)
{
...
{
    fmt::format_context::iterator format(const abstractions::OperationTiming &timing, fmt::format_context &ctx) const;
};
