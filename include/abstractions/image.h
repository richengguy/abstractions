...
constexpr uint8_t GetAlphaValue(const uint32_t pixel) { return GetComponent<3>(pixel); }
constexpr uint8_t GetRedValue(const uint32_t pixel) { return GetComponent<2>(pixel); }
constexpr uint8_t GetGreenValue(const uint32_t pixel) { return GetComponent<1>(pixel); }
constexpr uint8_t GetBlueValue(const uint32_t pixel) { return GetComponent<0>(pixel); }

constexpr void SetAlphaValue(const uint8_t alpha, uint32_t &pixel) { SetComponent<3>(alpha, pixel); }
constexpr void SetRedValue(const uint8_t red, uint32_t &pixel) { SetComponent<2>(red, pixel); }
constexpr void SetGreenValue(const uint8_t green, uint32_t &pixel) { SetComponent<1>(green, pixel); }
constexpr void SetBlueValue(const uint8_t blue, uint32_t &pixel) { SetComponent<0>(blue, pixel); }
...
/// @param a alpha value
/// @return packed value
constexpr uint32_t PackComponents(const uint8_t r, const uint8_t g, const uint8_t b, const uint8_t a)
{
    uint32_t pixel = 0;
    SetRedValue(r, pixel);
...
public:
    ...
    uint8_t Red() const { return detail::GetRedValue(_pixel); }
    uint8_t Green() const { return detail::GetGreenValue(_pixel); }
    uint8_t Blue() const { return detail::GetBlueValue(_pixel); }
    uint8_t Alpha() const { return detail::GetAlphaValue(_pixel); }
...
public:
    ...
    operator BLImage&() { return _buffer; }
