...
public:
    ...
    {
        _test_name = fmt::format("{}", fmt::styled(test_name, fmt::emphasis::faint | fmt::emphasis::italic));
    }
...
public:
        ...
        : _folder{kResultsPath / "s" / test_name}
    {
        ...
        if (std::filesystem::exists(_folder))
        {
            ...
            console.Print(fmt::format("Removed {} files.", fmt::styled(removed, fmt::emphasis::bold)));
        }

        auto ret = std::filesystem::create_directories(_folder);
        if (ret)
        {
            console.Print(fmt::format("Created {}", fmt::styled(_folder.c_str(), fmt::emphasis::bold)));
        }
    }
